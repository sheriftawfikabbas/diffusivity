...
class DiffusionCoefficient:
    ...
    def __init__(self, traj, timestep, atom_indices=None, molecule=False, calculation_type="tracer", axis='all'):
        ...
        if self.calculation_type == 'molecule':
    ...
    def _initialise_arrays(self, ignore_n_images, number_of_segments):
        ...
        if self.axis != 'xy':
            ...
            self.slopes = np.zeros(
                (self.no_of_types_of_atoms, self.no_of_segments, 2))
            self.intercepts = np.zeros(
                (self.no_of_types_of_atoms, self.no_of_segments, 2))
    ...
    def calculate(self, ignore_n_images=0, number_of_segments=1):
        ...
        for segment_no in range(self.no_of_segments):
            ...
            tracer_diffs = [[[0, 0, 0] for x in range(len(self.atom_indices))]]
            ...
            for image_no in range(0, len(seg)):
                ...
                for sym_index in range(self.no_of_types_of_atoms):
                    ...
                    if self.calculation_type == 'tracer' or self.calculation_type == 'molecule':
    ...
    def _fit_data(self, x, y):
        ...
        if self.axis == 'all':
            ...
            x_edited = np.vstack([np.array(x), np.ones(len(x))]).T
            slopes, intercepts = np.linalg.lstsq(x_edited, y[2], rcond=-1)[0]
        elif self.axis == 'x':
            x_edited = np.vstack([np.array(x), np.ones(len(x))]).T
            slopes, intercepts = np.linalg.lstsq(x_edited, y[0], rcond=-1)[0]
        elif self.axis == 'y':
            x_edited = np.vstack([np.array(x), np.ones(len(x))]).T
            slopes, intercepts = np.linalg.lstsq(x_edited, y[1], rcond=-1)[0]
        elif self.axis == 'xy':
            slopes = np.zeros(2)
    ...
    def plot(self, ax=None, show=False, save_fig=False, fig_name=''):
        """
        ...
        for segment_no in range(self.no_of_segments):
            ...
            ax.plot([x_coord, x_coord], [-0.001, 1.05 *
                    np.amax(self.xyz_segment_ensemble_average)], color='grey', linestyle=":")
        ...
        if show:
            plt.show()
        if save_fig:
            plt.savefig('ArrheniusPlot_'+fig_name, bbox_inches='tight')
